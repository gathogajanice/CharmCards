...
exports.getBitcoinRpcConfig = void 0;
exports.getBitcoinNodeHealth = getBitcoinNodeHealth;
exports.isBitcoinCoreReady = isBitcoinCoreReady;
exports.testBitcoinRpcConnection = testBitcoinRpcConnection;
...
exports.getBitcoinRpcConfig = getBitcoinRpcConfig;
async function callBitcoinRpc(method, params, config) {
 ...
 */
async function callBitcoinRpcWithTimeout(method, params, config, timeoutMs = 60000) {
    ...
    try {
        const response = await axios_1.default.post(rpcUrl, requestBody, {
            headers,
            timeout: timeoutMs,
        ...
        if (error.response) {
            // HTTP error response
            const rpcResponse = error.response.data;
            if (rpcResponse?.error) {
                throw new Error(`Bitcoin RPC error (${rpcResponse.error.code}): ${rpcResponse.error.message}`);
            }
        ...
        if (error.code === 'ECONNREFUSED') {
            throw new Error('Bitcoin RPC connection refused. ' +
 ...
 */
async function getBitcoinNodeHealth(config) {
    try {
 ...
 */
async function isBitcoinCoreReady(config) {
    const rpcConfig = config || (0, exports.getBitcoinRpcConfig)();
    if (!rpcConfig) {
        return {
            ready: false,
 ...
 */
async function testBitcoinRpcConnection(config) {
    try {
        ...
        if (error.message?.includes('timeout') || error.code === 'ECONNABORTED') {
            ...
            try {
                ...
                const pingResponse = await axios_1.default.post(config.url, {
                    jsonrpc: '2.0',
                    id: 999,
 ...
 */
async function getTransactionBlockHeight(txid) {
    try {
        const txUrl = `${MEMEPOOL_BASE_URL}/api/tx/${txid}`;
        const response = await axios_1.default.get(txUrl, { timeout: 10000 });
 ...
 */
async function checkNodeSyncForTransaction(txid, config) {
    try {
 ...
 */
async function verifyPackageTopology(commitTxHex, spellTxHex) {
    try {
 ...
 */
async function extractTransactionStructure(txHex) {
    try {
        const bitcoin = await Promise.resolve().then(() => __importStar(require('bitcoinjs-lib')));
        const network = NETWORK === 'testnet4' || NETWORK === 'testnet'
            ? { messagePrefix: '\x18Bitcoin Signed Message:\n', bech32: 'tb', bip32: { public: 0x043587cf, private: 0x04358394 }, pubKeyHash: 0x6f, scriptHash: 0xc4, wif: 0xef }
 ...
 */
async function submitTransactionsSequentially(commitTxHex, spellTxHex, config) {
 ...
 * @returns Object with transaction IDs for both transactions
 */
async function broadcastPackageViaBitcoinRpc(commitTxHex, spellTxHex, config) {
    ...
    try {
        ...
        for (const txid of txids) {
            const txResult = Array.isArray(result)
                ? result.find((item) => (item?.txid || item?.transactionId || item) === txid)
                : result[txid];
        ...
        if (isTopologyError) {
            ...
            try {
                ...
                const enhancedError = new Error(`Package topology error and sequential fallback failed. ` +
                    `Original error: ${error.message}. ` +
                    `Fallback error: ${fallbackError.message}`);
                enhancedError.originalError = error.message;
                enhancedError.fallbackError = fallbackError.message;
...
async function validateTransaction(txHex) {
    ...
    try {
        ...
        try {
            ...
            if (commitStructure.inputs.length > 0) {
                console.log(`     Input UTXOs: ${commitStructure.inputs.map((i) => i.ref).join(', ')}`);
            }
            ...
            if (spellStructure.inputs.length > 0) {
                console.log(`     Input UTXOs: ${spellStructure.inputs.map((i) => i.ref).join(', ')}`);
            }
        }
        catch (structError) {
            console.warn(`⚠️ Could not extract transaction structure: ${structError.message}`);
