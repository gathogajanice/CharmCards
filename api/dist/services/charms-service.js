 ...
 */
async function computeTxid(txHex) {
    try {
...
class CharmsService {
    ...
    async getAppVk() {
        ...
        try {
            ...
            try {
                ...
                throw new Error(`WASM file not found at: ${appBin}\n` +
                    `Please either:\n` +
     ...
     */
    async checkSpell(spellYaml, prevTxs) {
        try {
            ...
            if (stdout)
                console.log('Spell check output:', stdout);
            if (stderr && stderr.trim())
                console.warn('Spell check warnings:', stderr);
            return true;
        }
        catch (error) {
     ...
     */
    async generateProof(spellYaml, appBin, prevTxs, mockMode = false, fundingUtxo, fundingUtxoValue, changeAddress, feeRate = 2.0) {
        ...
        try {
            ...
            for (const [key, value] of Object.entries(spellObj.apps)) {
                ...
                if (!appIdPattern.test(value)) {
                    ...
                    throw new Error(`Invalid spell: App value for ${key} has invalid format "${value}". ` +
            ...
            Object.entries(spellObj.apps).forEach(([key, appId]) => {
            ...
            // Validate each input
            spellObj.ins.forEach((input, index) => {
            ...
            spellObj.outs.forEach((output, index) => {
            ...
            if (appBin) {
                try {
                    ...
                    try {
                        ...
                        throw new Error(`WASM binary not found at: ${resolvedAppBin}\n` +
                    ...
                    if (spellObj.apps && typeof spellObj.apps === 'object') {
                        ...
                        const missingBinaries = spellAppIds.filter((appId) => {
                            const appIdStr = String(appId);
                        ...
                        if (missingBinaries.length > 0 && !mockMode) {
                            ...
                            console.error(`   Missing app IDs: ${missingBinaries.map((id) => String(id)).join(', ')}`);
                            console.error(`   Binary keys provided: ${binaryKeys.length > 0 ? binaryKeys.join(', ') : 'NONE'}`);
                            throw new Error(`Missing binaries for app IDs: ${missingBinaries.map((id) => String(id)).join(', ')}. ` +
                ...
                if (!mockMode) {
                    throw new Error('App binary is required but not provided. Redeem operations require the binary to execute app logic. ' +
            ...
            if (!mockMode && Object.keys(binaries).length === 0) {
                throw new Error('Binaries object is empty but required for redeem operation. ' +
            ...
            const spellAppIds = spellObj.apps ? Object.values(spellObj.apps).map((id) => String(id)) : [];
            // Extract binary keys for logging (same format as what we send to Prover API)
            const binaryKeysFromSpell = spellAppIds.map((id) => {
            ...
            if (payload.prev_txs && payload.prev_txs.length > 0) {
                payload.prev_txs.forEach((tx, i) => {
                    const hex = typeof tx === 'string' ? tx : tx.bitcoin;
            ...
            if (payload.prev_txs && Array.isArray(payload.prev_txs)) {
                payload.prev_txs = payload.prev_txs.map((tx, index) => {
                    if (typeof tx === 'string') {
            ...
            if (payload.prev_txs && payload.prev_txs.length > 0) {
                payload.prev_txs.forEach((tx, index) => {
            ...
            const payloadSample = {
                ...
                ...payload,
                prev_txs: payload.prev_txs?.map((tx) => {
            ...
            if (payload.spell && payload.spell.apps) {
                ...
                Object.entries(payload.spell.apps).forEach(([key, value]) => {
            ...
            if (payload.binaries && Object.keys(payload.binaries).length > 0) {
                Object.entries(payload.binaries).forEach(([appVk, binary]) => {
            ...
            if (payload.prev_txs && Array.isArray(payload.prev_txs)) {
                const isValid = payload.prev_txs.every((tx) => {
                    return tx &&
                        typeof tx === 'object' &&
                        'bitcoin' in tx &&
            ...
            if (useCli) {
                try {
                    ...
                    const prevTxsHexStrings = prevTxsArray.map((tx) => tx.bitcoin);
            ...
            const debugPayload = {
                spell: {
                    ...
                    ins: payload.spell.ins?.map((inp) => ({
                        utxo_id: inp.utxo_id,
                        charms: Object.keys(inp.charms || {}).length + ' charm(s)'
                    })),
                    outs: payload.spell.outs?.map((out) => ({
                ...
                binaries: payload.binaries,
                prev_txs: payload.prev_txs?.map((tx) => `${tx.bitcoin.substring(0, 32)}... (${tx.bitcoin.length} chars)`),
                chain: payload.chain,
     ...
     */
    async loadSpellTemplate(templateName, variables) {
